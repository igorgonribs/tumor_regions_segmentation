...
from sourcecode.BONE_CHANNELS.bones_dataloader import BONE_CHANNELSDataset
...
class BONE_CHANNELS_Dataset512x512(BONE_CHANNELSDataset):

    def __init__(self,
                 img_dir="../../datasets/BONE_CHANNELS_reduced_size",
...
def load_dataset(img_dir, dataset_type):
    ...
    for root, d, _ in sorted(os.walk(dataset_root_dir)):
        ...
        for cls in sorted([cls for cls in d if cls in classes]):
            ...
            original_dir = "{}/{}".format(dataset_root_dir, cls)
            mask_dir = "{}/annotations".format(dataset_root_dir)

            for dirpath, dirnames, filenames in sorted(os.walk(original_dir)):
                for fname in sorted(filenames):

                    path_img = original_dir + '/' + fname
                    path_mask = mask_dir + '/' + fname

                    #if is_valid_file(path_img) and first_train.find(fname) < 0:
                    logger.info(path_img)
...
def create_dataloader(tile_size="512x512",
    ...
    image_datasets = {x: BONE_CHANNELS_Dataset512x512(img_dir=dataset_dir,
